...
class TestWindowsPaths:
    ...
    def test_long_path_handling(self, tmp_path):
        ...
        out = vhc.run_healthcheck(
            output_dir=str(deep), demo=True, verbose=False, write_artifacts=True,
        )
        assert out["findings"]
...
class TestWindowsServerVersions:
    ...
    def test_security_compliance_per_os(self, scenario, tmp_path):
        ...
        if scenario["rdp_status"] != "Passed":
            ...
            assert not any("Remote desktop" in f for f in sec_findings), f"False RDP finding on {scenario['os']}"
...
class TestIntegrationErrorHandling:
    ...
    def test_salesforce_missing_credentials(self, tmp_path):
        with mock.patch.dict(os.environ, {}, clear=True):
            out = vhc.run_healthcheck(
                output_dir=str(tmp_path), demo=True, verbose=False,
                write_artifacts=False, sf_account_id="001FAKE",
            )
    ...
    def test_valid_slack_webhook_format_accepted(self):
        ...
        assert not vhc._validate_slack_webhook("http://hooks.slack.com/services/T00/B00/xxx")
...
class TestMainEntrypoint:
    ...
    def test_main_returns_2_on_errors(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            sys, "argv",
            ["vhc_simplifier.py", "--input-dir", str(tmp_path / "nope"), "--quiet", "--no-artifacts"],
        )
        assert vhc.main() == 2
